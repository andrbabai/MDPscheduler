...
async def index(request: Request) -> str:
    ...
    return (
        ...
        ""
        ".calendar{width:100%;table-layout:fixed;border-collapse:collapse;border:1px solid #e0e0e0;border-radius:12px;overflow:hidden}"
        ".calendar th, .calendar td{border:1px solid #e0e0e0;padding:10px;vertical-align:top;word-break:break-word;overflow-wrap:anywhere}"
        ...
        ".cal-wrap{overflow-x:auto;-webkit-overflow-scrolling:touch;margin:4px -4px 0;padding:0 4px}"
        ...
        ".ev-item .ev-title{display:block;line-height:1.3;overflow-wrap:anywhere;word-break:break-word}"
        ".ev-item .ev-more{display:inline-block;padding:2px 6px;margin-top:2px;border:1px solid #e0e0e0;border-radius:6px;background:#fff;color:#2563eb;cursor:pointer;text-decoration:none;font-weight:600}"
        ".ev-time{color:#666;margin-right:4px}"
        ".cal-summary{display:block;cursor:pointer;padding:10px 12px;margin:-6px -6px 8px;border:1px solid #e0e0e0;border-radius:10px;background:#f8fafc;color:#111;font-weight:600}"
        ".cal-caret{display:inline-block;margin-left:8px;transition:transform .15s ease}"
        "details[open] .cal-caret{transform:rotate(180deg)}"
        "#ev-popover-backdrop{position:fixed;inset:0;background:rgba(0,0,0,.25);z-index:9998;display:none}"
        "#ev-popover{position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);max-width:90vw;max-height:70vh;overflow:auto;background:#fff;border:1px solid #e0e0e0;border-radius:12px;box-shadow:0 10px 30px rgba(0,0,0,.2);padding:16px 20px;z-index:9999;display:none}"
        "#ev-popover .pop-close{position:absolute;top:6px;right:8px;background:transparent;border:0;font-size:18px;cursor:pointer;color:#666}"
        ""
        ...
        "@media (max-width: 640px){ h1{font-size:22px} h2{font-size:18px} .calendar th,.calendar td{padding:6px} .ev-item{font-size:11px} body{margin:20px auto} .cal-head{margin:6px 0 8px} }"
        "a{color:#3a86ff;text-decoration:none} a:hover{text-decoration:underline}"
        "</style>"
        "<script>(function(){function q(s,r){return (r||document).querySelector(s)}function qa(s,r){return Array.prototype.slice.call((r||document).querySelectorAll(s))}function ensure(){var b=q('#ev-popover-backdrop');var p=q('#ev-popover');if(!b){b=document.createElement('div');b.id='ev-popover-backdrop';document.body.appendChild(b)}if(!p){p=document.createElement('div');p.id='ev-popover';p.innerHTML='<button class=\\'pop-close\\' aria-label=\\'Закрыть\\'>&times;</button><div class=\\'pop-content\\'></div>';document.body.appendChild(p)}return {backdrop:b,popover:p,content:q('.pop-content',p),close:q('.pop-close',p)}}var st=ensure();function hide(){st.backdrop.style.display='none';st.popover.style.display='none'}function show(t){st.content.textContent=t;st.backdrop.style.display='block';st.popover.style.display='block'}st.backdrop.addEventListener('click',hide);st.close.addEventListener('click',hide);document.addEventListener('keydown',function(e){if(e.key==='Escape')hide()});function setup(){qa('.ev-item').forEach(function(it){var t=q('.ev-title',it);if(!t)return;var lh=parseFloat(getComputedStyle(t).lineHeight)||16;var mx=5;if(t.scrollHeight>lh*mx+1){var full=t.textContent.trim();t.style.display='none';var a=document.createElement('a');a.href='javascript:void(0)';a.className='ev-more';a.textContent='Пары';a.setAttribute('role','button');a.addEventListener('mouseenter',function(){show(full)});a.addEventListener('mouseleave',function(){hide()});a.addEventListener('click',function(e){e.preventDefault();show(full)});it.appendChild(a);}})}if(document.readyState==='loading'){document.addEventListener('DOMContentLoaded',setup)}else{setup()}})();</script>"
        "</head>"
        "<body>"
        "<header>"
        ...
        "<summary class=\"cal-summary\">Календарь <span class=\"cal-caret\">▾</span></summary>"
